# plotFunctions.R - General Plotting Utilities for Seurat Objects
# =============================================================================
# Collection of plotting functions for single-cell RNA-seq visualization
# =============================================================================

#' @title Plotting utilities for scRNA-seq analysis
#' @description Functions for violin plots, bar plots, UMAP visualization,
#'   and cell composition analysis.
#' @author shycheng
...
# --- Load Configuration ---
# Source config.R for colors and paths (optional, falls back to defaults)
tryCatch(
  {
    source(file.path(dirname(sys.frame(1)$ofile), "config.R"))
  },
  error = function(e) {
    message("Note: config.R not loaded. Using default color palettes.")
  }
)
...
#' Modify violin plot for stacking
#' @description Helper function for stacked_vln_plot. Adjusts margins and styling.
#' @param obj Seurat object
#' @param feature Gene name to plot
#' @param pt.size Point size (default 0)
#' @param plot.margin Margin adjustment
...
barplot_per <- function(Seob, colValues = brewer.pal(7, "RdBu"), Cell_Type = "Cell_Type") {
  ...
  bar_plot <- function(df, position = "stack") {
    ...
    df_bar$day <- factor(df_bar$day, levels = c(
...
barplot_cluster <- function(seob, umap_plot) {
  ...
  df_test$Cell_type <- factor(as.character(rep(cell_type$Cell_type, n)),
    levels =
  ...
  df_test$day <- factor(df_test$day, levels = c(
    "26dpi_F", "22dpi_F", "18dpi_F", "14dpi",
    "18dpi_M", "22dpi_M", "26dpi_M"
  ))
...
plot_alluvium <- function(seob, umapplot, Cell_Type = "NewCellType", filename,
  ...
  p_alluvium <- ggplot(
    seob_CellProp,
    aes(
      x = day, stratum = Cell_type, alluvium = Cell_type,
      y = Freq,
...
# =============================================================================
# Load gene ID to name mapping (uses config.R if available)
id2name <- tryCatch(
  {
    if (exists("CONFIG") && !is.null(CONFIG$gene_info_file)) {
      load_gene_info(CONFIG)
    } else {
      gene_info_path <- Sys.getenv("SCATLAS_GENE_INFO",
        unset = "~/genome/Schitosoma_jp/Schistosoma_japonicum/SJ_GODB/sj_geneInfo_raw.tsv"
      )
      if (file.exists(gene_info_path)) {
        df <- read.delim(gene_info_path, sep = "\t", header = FALSE)
        colnames(df) <- c("id", "Gene_Name", "Note")
        rownames(df) <- df$id
        df$Gene_Name <- as.character(df$Gene_Name)
        idx <- which(nchar(df$Gene_Name) > 13)
        df$Gene_Name[idx] <- rownames(df[idx, ])
        df$Note <- gsub("\\(.*\\)", "", df$Note)
        df$id <- gsub("_", "-", df$id)
        rownames(df) <- df$id
        df
      } else {
        message("Gene info file not found. Some functions may not work.")
        NULL
      }
    }
  },
  error = function(e) {
    message("Could not load gene info: ", e$message)
    NULL
  }
)
...
plot_umap <- function(seob,
  ...
  # 绘制UMAP散点图
  plot(umap[rand_ind, 1], umap[rand_ind, 2],
    pch = 16, xlab = "UMAP-1", ylab = "UMAP-2",
    frame = F, col = col_pal, cex = point_size
  )

  # 添加图例
  legend("topright",
    legend = cell_levels, fill = celltypes_pal,
